# Contributing

Thank you for helping improve tokenc.

## Development setup

Requirements:

- Node.js 22.13 or newer
- pnpm 11

```bash
pnpm install
pnpm check
pnpm build
```

## Pull requests

1. Keep changes focused and preserve package boundaries.
2. Add tests for behavior changes.
3. Run `pnpm check` and `pnpm build` before opening a pull request.
4. Run `pnpm changeset` for changes that affect a published package.
5. Update English and Chinese documentation together when public behavior changes.

The compiler core must not print to the terminal, terminate the process, or write output artifacts. Those responsibilities belong to the CLI and IO layers.

## Commit and release model

The repository uses Changesets. Public packages are kept in a fixed version group during the early release phase. Maintainers create versions with `pnpm version-packages` and publish with `pnpm release`.

## 中文说明

提交代码前请确保 `pnpm check` 与 `pnpm build` 均通过。影响公开 package 行为的修改需要执行 `pnpm changeset`。如果修改了公开文档，请同步维护英文与简体中文版本。
